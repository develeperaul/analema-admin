<template>
  <div class="tw-px-6 tw-py-4 tw-bg-white">
    ...
    >
      <h3 class="">Комерческое преложение</h3>
    ...
    >
      ...
      >
        ...
        <app-input
          ...
          v-model="summModel"
        />
        <app-button text="Отправить комерческое предложение" @click="send" />
      </div>
    </div>
  </div>
</template>
...
const send = async () => {
  ...
  try {
    await store.update({
      id: String(offerProps.id),
      documents: idList,
      summ: summModel.value,
    });
...
onMounted(() => {
  if (offerProps.summ) summModel.value = offerProps.summ;
  if (cardWrapper.value)
    wrapperHeight.value = cardWrapper.value.getBoundingClientRect().height;
